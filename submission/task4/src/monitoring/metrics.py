...
Task 4 restores the full metric set used by the live-monitoring Grafana
dashboard. The service emits cheap request signals on every /chat request,
and the judge worker emits sampled deep-quality signals.
"""
...
chat_input_tokens = Histogram(
    "chat_input_tokens",
    "Input token count distribution for each model call made by /chat.",
    ["config_id", "model"],
...
chat_output_tokens = Histogram(
    "chat_output_tokens",
    "Output token count distribution for each model call made by /chat.",
    ["config_id", "model"],
...
judge_evaluations_total = Counter(
    ...
    "Completed sampled judge evaluations, sliced by judge verdict.",
    ["config_id", "verdict"],
)

judge_latency_seconds = Histogram(
    "judge_latency_seconds",
    "Latency of sampled judge evaluations in seconds.",
    ["config_id"],
